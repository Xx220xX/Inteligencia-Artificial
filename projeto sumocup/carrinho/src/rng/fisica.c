...
#define Pprint(name, p)printf("%s = (%.2lf,%.2lf)\n",name,p.x*mscl,p.y*mscl)
#define PprintI(name, id, p)printf("%s%d = (%.2lf,%.2lf)\n",name,id,p.x*mscl,p.y*mscl)
Reta  RetaByPoints(P p1, P p2){
    return (Reta){Psub(p2,p1),p2};
}
double RetaAngle(Reta r){
    return Pangle(r.diretor);
}
void plotClear() {
...
P ProtEixo(P p, double radAngle) {
    P ans = {p.x * cos(radAngle) - p.y * sin(radAngle), p.x * sin(radAngle) + p.y * cos(radAngle)};
    return ans;
}
 ...
 * @see doc/FISICA.md
 */
void getCorners(P coners[4], P c1, P p1, double larg, double comp) {
    double angle = Pangle(Psub(p1, c1));
    double default_angle = atan2(larg, comp);
    // vetor direita cima
    P R_U = PmulEsc(PbyAngle(angle + M_PI_2 - default_angle), sqrt(larg * larg + comp * comp) / 2);
    P L_U = PmulEsc(PbyAngle(angle + M_PI_2 + default_angle), sqrt(larg * larg + comp * comp) / 2);

    coners[0] = Psub(c1, L_U);
    coners[1] = Psub(c1, R_U);
    coners[2] = Padd(c1, L_U);
    coners[3] = Padd(c1, R_U);
}
...
*/
    ...
    getCorners(coners1, c1, p1, largura, comprimento);
    getCorners(coners2, c1, p1, largura, comprimento);
    copyVet(coners1b, coners2, 0, 4);
    copyVet(coners2b, coners1, 0, 4);

    //  considerando carrinho 1
    // transladar para pa
    // pa eh o elemento 3 do vetor
    pa = coners1[2];
    pa1 = coners1b[2];
    for (i = 0; i < 4; i++) {
    ...
    angle = Pangle(coners1[2]);
    angleb = Pangle(coners1b[2]);
    for (i = 0; i < 4; i++) {
...
SolucaoCirculo encontroRetaCirculo(P centro, double Raio, P p0, P p1) {
    P r0 = Psub(p1, p0);
    P rp = Psub(p1, centro);

    double a = r0.x * r0.x + r0.y * r0.y,
            b = r0.x * rp.x + r0.y * rp.y,
            c = rp.x * rp.x + rp.y * rp.y - Raio * Raio;
    ...
    s.t0 = (-b + delta) / a;
    s.t1 = (-b + delta) / a;
    s.p0 = Padd(PmulEsc(r0), s.t0, p1);
    s.p1 = Padd(PmulEsc(r0), s.t1, p1);
    return s;
}
